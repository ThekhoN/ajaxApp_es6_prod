...
/***/ function(module, exports, __webpack_require__) {
	...
	//domLogX99
	var errorLog3 = document.getElementById('errorLog3');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Use_secondResponseData = __webpack_require__(35);

	var _Use_secondResponseData2 = _interopRequireDefault(_Use_secondResponseData);
	...
	var Use_firstResponseData = function Use_firstResponseData(data, O_O) {
	  ...
	  //update firstResponseData & clone to secondResponseData
	  //test
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _userAgentCheck_android = __webpack_require__(41);
	...
	*/
	var createHTML_by_categoryNames = function createHTML_by_categoryNames(O_O, dataForRender) {
	  //nonPromise, showSoldOut_g, finalHTML, dom_categoryNames
	  var firstUrl = O_O.firstUrl;
	  var pogIdList_arr = O_O.pogIdList_arr;
	  var nonPromise = O_O.nonPromise;
	  //init myFastdom
	  ...
	  //console.log('updated finalHTML: ', finalHTML);
	  if (nonPromise === true) {
	    ...
	    myFastdomX99.mutate(function () {
	      ...
	      console.log('first fastdom setHTML completed ');
	      ...
	      if (pogIdList_arr.length < 1) {
	        ...
	        console.log('O_O inside fastdom: ', O_O);

	        var Url_secondReq = (0, _getUrl_secondRequest2.default)(firstUrl, pogIdList_arr);

	        if ((0, _userAgentCheck_android2.default)()) {
	          console.log('this is android442');
	          Url_secondReq = 'https://m.snapdeal.com/acors/json/getProductById?pogIds=' + pogIdList_arr.join(',');
	        } else {
	          console.log('this is not android442');
	        }

	        errorLog5.innerHTML = 'Url_secondReq: ' + Url_secondReq;
	        console.log('inside fastdom Url_secondReq: ', Url_secondReq);

	        _axios2.default.get(Url_secondReq).then(function (response) {
	          //console.log(response.data);
	          var data = response.data;
	          errorLog4.innerHTML = 'typeof secondReq.data: ' + (typeof data === 'undefined' ? 'undefined' : _typeof(data)) + ' <br/>\n                secondReq.data.length: ' + data.length + '\n                ';
	          console.log('second req data: ', data);
	          ...
	          console.log('dataForRender inside fastdom: ', dataForRender);
	        }).catch(function (err) {
	          console.log('err in axios second req: ', err);
	          if (err.response) {
	            errorLog3.innerHTML = '<h3>Error in axios second req: </h3> <br/>\n                  <p> The request was made, but the server responded with a status code </p> <br/>\n                  error: ' + err + ' <br/>\n                  error.response.data: ' + err.response.data + ' <br/>\n                  error.response.status: ' + err.response.status + ' </br>\n                  error.response.headers: ' + err.response.headers + ' <br/>\n                  error.message: ' + err.message + ' <br/>\n                   ';
	          } else {
	            errorLog3.innerHTML = '<h3>Error in axios second req: </h3> <br/>\n                  <p> Something happened in setting up the request that triggered an Error <br/>\n                  error: ' + err + ' <br/>\n                  error.message: ' + err.message + ' <br/>\n                   ';
	          }
	        });
	      }
	    }).catch(function (err) {
...
/***/ function(module, exports) {
	...
	function userAgentCheck_android442(ua) {
	  var _getAndroidVersionX99 = getAndroidVersionX99(ua);
	  var version0;
	  //if android
	  if (_getAndroidVersionX99) {
